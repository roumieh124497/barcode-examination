...
const getSegmentsFromVectors = vectors =>
  getWithoutLastElement(vectors).map(
    (one, index) => new Segment(one, vectors[index + 1]),
  );
// #endregion
...
const getFood = (width, height, snake) => {
  const allPositions = getRange(width)
    .map(x => getRange(height).map(y => new Vector(x + 0.5, y + 0.5)))
    .flat();
  const segments = getSegmentsFromVectors(snake);
  const freePositions = allPositions.filter(point =>
    segments.every(segment => !segment.isPointInside(point)),
  );
  return getRandomFrom(freePositions);
};

const getGameInitialState = (config = {}) => {
  const { width, height, speed, initialSnakeLength, initialDirection } = {
    ...config,
    ...DEFAULT_GAME_CONFIG,
  };
...
const getNewTail = (oldSnake, distance) => {
  const { tail } = getWithoutLastElement(oldSnake).reduce(
    (acc, point, index) => {
...
const getStateAfterMoveProcessing = (state, movement, distance) => {
  ...
  if (!state.direction.equalTo(newDirection)) {
    ...
    const [oldXRounded, oldYRounded, newXRounded, newYRounded] = [
      oldX,
      oldY,
      newHead.x,
      newHead.y,
    ].map(Math.round);
    const getStateWithBrokenSnake = (
      old,
      oldRounded,
      newRounded,
      getBreakpoint,
      ...
      const breakpointComponent =
        oldRounded + (newRounded > oldRounded ? 0.5 : -0.5);
      const breakpoint = getBreakpoint(breakpointComponent);
      const vector = newDirection.scaleBy(
        distance - Math.abs(old - breakpointComponent),
      );
...
const isGameOver = ({ snake, width, height }) => {
  ...
  return getSegmentsFromVectors(tail)
    .slice(2)
    .find(segment => {
...
const renderCells = (context, cellSide, width, height) => {
  ...
  getRange(width).forEach(column =>
    getRange(height).forEach(row => {
      if ((column + row) % 2 === 1) {
...
const render = ({
  game: { width, height, food, snake, score },
  bestScore,
  projectDistance,
  projectPosition,
}) => {
...
const startGame = () => {
  ...
  window.addEventListener('resize', () => {
    ...
    updateState({
      ...containerSize,
      ...getProjectors(containerSize, state.game),
    });
    tick();
  });
  window.addEventListener('keydown', ({ which }) => {
    const entries = Object.entries(MOVEMENT_KEYS);
    const [movement] = entries.find(([, keys]) => keys.includes(which)) || [
      undefined,
    ];
  ...
  window.addEventListener('keyup', ({ which }) => {
    ...
    if (which === STOP_KEY) {
      ...
      if (state.stopTime) {
        updateState({
          stopTime: undefined,
          lastUpdate: state.time + now - state.lastUpdate,
        });
      } else {
